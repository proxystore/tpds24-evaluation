...
PORT=59465
redis-server --port $PORT --save "" --appendonly no --client-output-buffer-limit "pubsub 1024GB 1024GB 1" --maxmemory 0 &> /dev/null &
REDIS=$!
...
ARGS+="--data-size-bytes 10000 100000 1000000 10000000 50000000 "
ARGS+="--task-sleep 1.0 "
ARGS+="--repeat 1 "
